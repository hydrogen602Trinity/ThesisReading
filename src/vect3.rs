...
mod tests {
    ...
    fn vect_test_1() {
        assert_eq!(
            Vect3::new(1., 2., 3.) + Vect3::new(4., 5., 6.,),
            Vect3::new(5., 7., 9.)
        );

        assert_eq!(Vect3::new(1., 2., 3.) * Vect3::new(4., 0., 6.,), 22.);

        // ^       ^   ^
        // x cross y = z
        assert_eq!(
            Vect3::new(1., 0., 0.).cross(&Vect3::new(0., 1., 0.,)),
            Vect3::new(0., 0., 1.)
        );

        assert_eq!(Vect3::new(1., 2., 3.).mag_sq(), 14.);

        assert!((Vect3::new(1., 2., 3.).mag() - (14 as f64).sqrt()) < 0.000001);

        assert_eq!(
            Vect3::new(1., 2., 3.).norm(),
            Vect3::new(1., 2., 3.) / (14 as f64).sqrt()
        );
    }
}
